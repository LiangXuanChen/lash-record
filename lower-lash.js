...
(() => {
  ...
  style.textContent = `
    ...
    .lower-lash-fields{
      display:grid;
      grid-template-columns:minmax(120px,.7fr) minmax(160px,1fr) minmax(160px,1fr);
      gap:12px;
      align-items:stretch;
    }
    .lower-lash-field{
    ...
    @media(max-width:767px){
      ...
      .lower-lash-indicator{font-size:14px;margin:-20px auto 2px}
    }
  `;
  ...
  lengthSelect.innerHTML = lowerLengthOptions.map(value =>
    `<option value="${value}">${value} mm</option>`
  ).join('');
